...
func FormatBalanceChange(balance *int64, currency string) template.HTML {
	...
	if currency == "BOA" {
		...
		if balanceF < 0 {
			return template.HTML(fmt.Sprintf("<span title=\"%.0f GWei\" data-toggle=\"tooltip\" class=\"text-danger\">%.5f BOA</span>", float64(*balance), balanceF))
		}
		return template.HTML(fmt.Sprintf("<span title=\"%.0f GWei\" data-toggle=\"tooltip\" class=\"text-success\">+%.5f BOA</span>", float64(*balance), balanceF))
	} else {
		if balance == nil {
			return template.HTML("<span> 0 " + currency + "</span>")
		}
...
func FormatEth1Address(addr []byte) template.HTML {
	...
	eth1ExplorerAddr := Config.Frontend.Eth1Explorer
	return template.HTML(fmt.Sprintf("<a href=\"%s/address/0x%x\" class=\"text-monospace\">%s…</a>%s", eth1ExplorerAddr, addr, eth1Addr.Hex()[:8], copyBtn))
}
...
func FormatEth1Block(block uint64) template.HTML {
	...
	eth1ExplorerAddr := Config.Frontend.Eth1Explorer
	return template.HTML(fmt.Sprintf("<a href=\"%[2]s/block/%[1]d\">%[1]d</a>", block, eth1ExplorerAddr))
}
...
func FormatEth1BlockHash(block []byte) template.HTML {
	...
	eth1ExplorerAddr := Config.Frontend.Eth1Explorer
	return template.HTML(fmt.Sprintf("<a href=\"%[2]s/block/%#[1]x\">%[1]x</a>%s", block, eth1ExplorerAddr, copyBtn))
}
...
func FormatEth1TxHash(hash []byte) template.HTML {
	...
	eth1ExplorerAddr := Config.Frontend.Eth1Explorer
	return template.HTML(fmt.Sprintf(`<i class="fas fa-male mr-2"></i><a style="font-family: 'Roboto Mono'" href="%s/tx/0x%x">0x%v…</a>%v`, eth1ExplorerAddr, hash, hex.EncodeToString(hash)[:6], copyBtn))
}
...
func FormatEth1AddressWithName(address []byte, name string) template.HTML {
	...
	if name != "" {
		eth1ExplorerAddr := Config.Frontend.Eth1Explorer
		return template.HTML(fmt.Sprintf("<a href=\"%s/address/0x%x\" class=\"text-monospace\">%s</a>", eth1ExplorerAddr, eth1Addr, name))
	} else {
...
func FormatValidatorName(name string) template.HTML {
	...
	return template.HTML(fmt.Sprintf("<b><abbr title=\"This name has been set by the owner of this validator. Pool tags have been set by the www.agorascan.io team.\">%s</abbr></b>", str))
}
